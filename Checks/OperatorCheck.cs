...
public class OperatorCheck
{
    ...
    private static void CheckSegments(AnalyzerContext context, IEnumerable<Segment> segments,
        Dictionary<DefinedVariable, Segment> constantDrivers, bool thread = false)
    {
        foreach (var segment in segments)
        {
            ...
            foreach (var child in segment.Children)
            {
                ...
                {
                    case "=":
                        if (AnalyzerHelper.SearchConcatParent(segment) is
                            {
                            ...
                            context.Diagnostics.Add(new GenericAnalyzerDiagnostic(context,
                                $"Invalid Operator '=' for {variableEquals.IoType} {segment}. Cannot read from Output, use Buffer instead",
                    ...
                    case ":=":
                        if (AnalyzerHelper.SearchConcatParent(segment) is
                            ...
                            context.Diagnostics.Add(new GenericAnalyzerDiagnostic(context,
                                $"Invalid Operator := for {variableIo.VariableType} {segment}. Use <= instead",
                                DiagnosticLevel.Error, child.ConcatOperatorIndex,
                                child.ConcatOperatorIndex + child.ConcatOperator.Length));
                        break;
                    case "<=" when !AnalyzerHelper.InParameter(segment):
                            ...
                            } recordParent2 && AnalyzerHelper.SearchVariable(recordParent2) is { } variable)
                        {
                            if (variable.VariableType is not (VariableType.Io or VariableType.Signal
                                ...
                                context.Diagnostics.Add(new GenericAnalyzerDiagnostic(context,
                                    $"Invalid Operator <= for {variable.VariableType} {segment}. Use := instead",
                                    DiagnosticLevel.Error, child.ConcatOperatorIndex,
                                    child.ConcatOperatorIndex + child.ConcatOperator.Length));
                            else if (variable is DefinedIo { IoType: IoType.In })
                                context.Diagnostics.Add(new GenericAnalyzerDiagnostic(context,
                                    $"Invalid Operator <= for {variable.VariableType} {segment}. Cannot assign a value to an input",
                                    DiagnosticLevel.Error, child.ConcatOperatorIndex,
                                    child.ConcatOperatorIndex + child.ConcatOperator.Length));
                            ...
                            if (variable.VariableType is VariableType.Signal or VariableType.Io &&
                                ...
                                if (AnalyzerHelper.SearchTopSegment(segment, SegmentType.Process, SegmentType.Main,
                                        SegmentType.Component) is { } topLevel)
                                {
                                    if (constantDrivers.ContainsKey(variable))
                                    {
                                        if ((constantDrivers[variable] != topLevel ||
                                            ...
                                            AnalyzerHelper.SearchTopSegment(segment, SegmentType.Generate) == null)
                                            context.Diagnostics.Add(new GenericAnalyzerDiagnostic(context,
                                                $"Multiple constant drivers for {segment}. You can only drive a signal from one process",
                                                DiagnosticLevel.Error, child.ConcatOperatorIndex,
                                                child.ConcatOperatorIndex + child.ConcatOperator.Length));
                                    }
                                    else
            ...
            CheckSegments(context, segment.Children, constantDrivers,
                thread || segment.SegmentType is SegmentType.Thread);
