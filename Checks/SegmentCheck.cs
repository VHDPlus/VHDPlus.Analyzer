...
public static class SegmentCheck
{
    ...
    private static void CheckSegments(AnalyzerContext context, IEnumerable<Segment> segments, bool thread = false)
    {
        foreach (var segment in segments)
        {
            ...
            foreach (var child in segment.Children)
            {
                ...
                if (thread || segment.SegmentType is SegmentType.Thread or SegmentType.SeqFunction)
                {
                    ...
                    context.Diagnostics.Add(new GenericAnalyzerDiagnostic(context,
                        $"Invalid Segment {child.SegmentType}  inside Thread at {segment.SegmentType}", DiagnosticLevel.Warning, child));
                }
                else 
                {
                    if (ValidSegments[segment.SegmentType].Contains(child.SegmentType)) continue;
                    context.Diagnostics.Add(new GenericAnalyzerDiagnostic(context,
                        $"Invalid Segment {child.SegmentType} at {segment.SegmentType}", DiagnosticLevel.Warning, child));
                }
            }
            ...
            foreach (var parameter in segment.Parameter)
            {
                ...
                context.Diagnostics.Add(new GenericAnalyzerDiagnostic(context,
                    $"Invalid Segment {parameter.First().SegmentType} inside Parameter at {segment.SegmentType} ", DiagnosticLevel.Warning, parameter.First()));
            }
            CheckSegments(context, segment.Children, thread || segment.SegmentType is SegmentType.Thread or SegmentType.SeqFunction);
        }
    }
    ...
    public static IEnumerable<SegmentType> GetValidSegments(Segment start, bool parameter)
    {
        ...
        return AnalyzerHelper.SearchTopSegment(start, SegmentType.Thread, SegmentType.SeqFunction) == null 
            ? GetValidSegments(start.SegmentType) : GetValidSegmentsThread(start.SegmentType);
    }
}