...
public static class Analyzer
{
    ...
    public static AnalyzerContext Analyze(AnalyzerContext context, AnalyzerMode mode, ProjectContext? pC)
    {
        ...
        if (mode is AnalyzerMode.Full or AnalyzerMode.ErrorCheck)
        {
    ...
    private static void ResolveMissingTypes(AnalyzerContext context, IDictionary<string, DataType> customTypes)
    {
        for (var i = 0; i < context.UnresolvedTypes.Count; i++)
        {
            if (context.UnresolvedTypes[i].SegmentType is SegmentType.Return)
            {
                ...
                if (context.UnresolvedTypes[i].SegmentType is SegmentType.Return)
                {
                    var owner = ParserHelper.GetVariableOwner(customTypes, context.UnresolvedTypes[i], VariableType.Variable);
                    if (owner is Segment { SegmentType: SegmentType.Function } funct && AnalyzerHelper.SearchFunction(context.UnresolvedTypes[i], funct.LastName) is {} functOwner)
                    {
            ...
            if (variableOwner is Segment { SegmentType: SegmentType.SeqFunction } seqFunc && AnalyzerHelper.SearchSeqFunction(context.UnresolvedTypes[i], seqFunc.LastName) is {} seqOwner)
            {
                var par = seqOwner.Parameters.FirstOrDefault(x => x.Name.Equals(varName, StringComparison.OrdinalIgnoreCase));
                if (par != null)
                {
            ...
            if (variableOwner is Segment { SegmentType: SegmentType.Function } func && AnalyzerHelper.SearchFunction(context.UnresolvedTypes[i], func.LastName) is {} funcOwner)
            {
                var par = funcOwner.Parameters.FirstOrDefault(x => x.Name.Equals(varName, StringComparison.OrdinalIgnoreCase));
    ...
    private static void ResolveMissingSegments(AnalyzerContext context)
    {
        for (var i = 0; i < context.UnresolvedSegments.Count; i++)
        {
            ...
            if (variable == null && context.AvailableExposingVariables.ContainsKey(context.UnresolvedSegments[i].NameOrValue.ToLower()))
                variable = context.AvailableExposingVariables[context.UnresolvedSegments[i].NameOrValue.ToLower()];
            if (variable == null)
            {
                var function = AnalyzerHelper.SearchFunction(context.UnresolvedSegments[i], context.UnresolvedSegments[i].NameOrValue);
            ...
            else
            {
                ...
                if(context.UnresolvedSegments[i].SegmentType is SegmentType.Unknown) context.UnresolvedSegments[i].SegmentType = SegmentType.DataVariable;
            }
        ...
        for (var i = 0; i < context.UnresolvedSeqFunctions.Count; i++)
        {
            if (context.UnresolvedSeqFunctions[i].NameOrValue.Split(' ') is { Length: 2 } comp)
            {
                ...
                if (AnalyzerHelper.SearchSeqFunction(context.UnresolvedSeqFunctions[i], compName) is {} seqFunc)
                {
                    ...
                    {
                        if (seqFunc.Parameters.FirstOrDefault(x => x.Name == seqVariable.Key) is {} parameter && context.UnresolvedSeqFunctions[i].Parameter.Any())
                        {
                            var variableOwner = ParserHelper.GetVariableOwner(context.AvailableTypes, context.UnresolvedSeqFunctions[i],
                                seqVariable.Value.VariableType);

                            var parIndex = seqFunc.Parameters.IndexOf(parameter);
                            var funcPar = context.UnresolvedSeqFunctions[i].Parameter[0].Any() ? context.UnresolvedSeqFunctions[i].Parameter[0][0] : null;
                                ...
                                if (c == parIndex)
                                {
                                    ...
                                    else
                                    {
                                        context.Diagnostics.Add(new GenericAnalyzerDiagnostic(context,
                                            $"{newName} already defined in {variableOwner}", DiagnosticLevel.Error, funcPar.Offset,
                            ...
                            if (funcPar == null)
                            {
                                ...
                                {
                                    context.Diagnostics.Add(new GenericAnalyzerDiagnostic(context,
                                        $"{seqVariable.Key} already defined in {variableOwner}", DiagnosticLevel.Error, context.UnresolvedSeqFunctions[i].Offset,
                                        context.UnresolvedSeqFunctions[i].Offset + context.UnresolvedSeqFunctions[i].NameOrValue.Length));
                                }
                            }
                        }
                    }
